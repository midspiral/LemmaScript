//@ backend dafny
// Idiomatic TypeScript domain — JSON-native (no Map/Set)
...
function listNameExists(m: Model, name: string, excludeList?: ListId): boolean {
  ...
  while (i < m.lists.length) {
    ...
    if (excludeList === undefined || lid !== excludeList) {
      const existing = m.listNames[lid]
