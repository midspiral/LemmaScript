...
function parseOptionalCheck(cond: TExpr): { scrutinee: TExpr; innerTy: Ty; negated: boolean; binderHint: string; truthiness: boolean } | null {
  ...
  if (cond.kind === "unop" && cond.op === "!" && cond.expr.ty.kind === "optional") {
    ...
    return { scrutinee: e, innerTy, negated: true, binderHint: mintName(hint, { specVisible: true }), truthiness: true };
  }
  if (cond.kind !== "binop" || (cond.op !== "!==" && cond.op !== "===")) {
    ...
    if (cond.ty.kind === "optional") {
      ...
      if (hint === null) return null;
      return { scrutinee: cond, innerTy: cond.ty.inner, negated: false, binderHint: mintName(hint, { specVisible: true }), truthiness: true };
    }
    return null;
  }
  ...
  const hint = binderHintFor(e);
  if (hint === null) return null;
  return { scrutinee: e, innerTy: e.ty.inner, negated: cond.op === "===", binderHint: mintName(hint, { specVisible: true }), truthiness: false };
}
...
function ruleNullish(e: TExpr): TExpr | null {
  ...
  const binder = mintName(`_oc${_ocCounter++}_val`);
  return {
    kind: "someMatch",
...
function binderHintForMapAccess(m: TExpr, k: TExpr): string {
  ...
  if (mHint && kHint) {
    ...
    return mintName(`${mStem}_${kStem}_val`, { specVisible: true });
  }
  return mintName(`_oc${_ocCounter++}_val`);
}
...
function ruleConditionalOptionalTruthy(e: TExpr): TExpr | null {
  ...
  if (hint === null) return null;
  const binder = mintName(hint, { specVisible: true });
  return {
    kind: "someMatch",
