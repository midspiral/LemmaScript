...
import { generateDef, generatePureDefs } from "./codegen.js";
...
function main() {
  ...
  // Generate .types.lean (type declarations + pure function mirrors)
  const typesContent = generateTypesLean(mod.typeDecls);
  const pureDefsLines = generatePureDefs(mod);
  const hasTypes = typesContent || pureDefsLines.length > 0;
  const typesImport = hasTypes ? `«${base}.types»` : undefined;
  if (hasTypes) {
    const header = typesContent ?? "/-\n  Generated by lsc — Lean types and pure function mirrors.\n-/\nimport Velvet.Syntax\nimport Velvet.Std\n";
    const typesPath = path.join(dir, `${base}.types.lean`);
    writeFileSync(typesPath, header + pureDefsLines.join("\n") + "\n");
