...
- **F* (experimental)** — a generated `.fst.gen` and an additions-only working `.fst`, named from the source path. Higher-order functions and the shared mathematical value model; see [SPEC_FSTAR.md](SPEC_FSTAR.md).
